   ...
   <section role="main" class="content-body">
    <header class="page-header">
      ...
      <div class="right-wrapper pull-right" style=" margin-right: 50px; ">
       ...
       @foreach($EditPageStationApply as $key => $StationApply)
    <form action="{{url('/EditStationApply')}}" method="POST" name="EditStationApplyForm">
      {{ csrf_field() }}
      <div class="panel-body">
          ...
          <div class="form-group">
            ...
            <div class="col-sm-9">
              <input type="number" name="stationapplyno" id="stationapplyno" class="form-control" min="1" value="{{$StationApply->stationapplyno}}" required/>
              ...
              <input type="hidden" name="stationapplyid" id="stationapplyid" class="form-control" value="{{request()->route('stationapplyid')}}" required/>
            </div>
          </div>
          ...
          <div class="col-md-6">
              <select data-plugin-selectTwo class="form-control populate" name="stationid" id="stationid" required>
                ...
                @foreach($StationList as $key => $Station)
                  <option {{ ($StationApply->stationid == $Station->stationid) ? "selected" : "" }} value="{{$Station->stationid}}">{{$Station->stationname}}</option>
                @endforeach
              </select>
            </div>
          </div>
